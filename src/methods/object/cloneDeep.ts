 ...
 */
export const cloneDeep = (souceData: any) => {
  // 不符合要求的数据抛回异常
  if (!souceData || typeof souceData !== 'object') return souceData;
  const resultData: any = Array.isArray(souceData) ? [] : {}
  for (const key in souceData) {
