 ...
 */
export var _easeBucketSort = function (numArr, direction) {
    if (direction === void 0) { direction = true; }
    // 非数组则抛出异常
    if (!Array.isArray(numArr))
        throw new TypeError('params are not required arrays');
    // 长度小于等于1位则无需排序
    if (numArr.length <= 1)
        return numArr;
    var cacheArr = [], resultArr = [];
